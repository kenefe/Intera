...
package com.kenefe.folme {
	...
	public class SpeedXYTracker2 extends MovieClip {
				...
				for(var i = history.length - 1; i >= 0; i--){
					...
					if(deltaTime > minValidTime){
						...
						if(
							...
							if(tmpVelocity.distance > velocity.distance) {
								velocity = tmpVelocity;
								if(lastVelocity.distance != 0){
									velocity.x = velocity.distance / lastVelocity.distance * lastVelocity.x;
									velocity.y = velocity.distance / lastVelocity.distance * lastVelocity.y;
								}
							}
							break;
						}
