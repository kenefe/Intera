...
| 2026-02-15 | states+patch R3 | Patch 端口拖线失败 — drag 操作无法在端口间创建连线，pointerdown 的 e.target 未命中 .port-dot (P0) | — | 🔴 |
...
| P0 | **Patch 端口拖线失败**: journey-server drag 操作（page.mouse.down→move→up）无法在端口间创建连线。hover 时端口高亮正常说明坐标准确，但 pointerdown 的 e.target 未命中 .port-dot 元素。阻断 {states,patch} 画像交互链路搭建 | states+patch R3 | 需调查 .port-dot 的 pointer event 命中检测 |
| P3 | 无撤销操作视觉反馈 | 用户 A | |
