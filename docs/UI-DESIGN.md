...
- 选中图层高亮，多选支持 (Shift+点击, Cmd+点击, 画布框选)
...
|------|------|------|
| frame | □ | 蓝色 |
| rect / ellipse / text | 对应形状 | 白色 |
| instance | ◇ 菱形 | 粉色 `#f472b6` |
| component (master) | ◆ 实心菱形 | 绿色 `#6ee7b7` |
...
| 通用 | 复制 / 删除 / 重命名 |
| frame | + 创建组件 |
| instance | 进入 Master / 解除关联 (Detach) |
| 任意选中 | ⚡按钮反馈 / ⚡卡片展开 (Sugar 预设) |
...
| 点击画板 | 切换到该状态进行编辑 |
| 点击画板内图层 | 选中图层 |
| 在画板空白区拖拽 | 框选图层 (与选择框相交即选中) |
| Alt+点击图层 | 穿透选择 — 跳过当前选中图层，命中下层重叠图层 |
| 在画板内右键图层 | 弹出图层上下文菜单 (复制 / 创建组件 / 删除) |
| Space+拖拽 | 平移画布 |
| Cmd+滚轮 / 双指缩放 | 缩放画布 |
| Cmd+0 | 缩放到适合全部内容 |
| Cmd+1 | 缩放到 100% |
| 绘制工具+画板内拖拽 | 在该状态的画板内创建图层 |
| 绘制工具+Shift松手 | 保持当前绘制工具，方便连续绘制多个同类图形 |
...
#### Instance 属性面板 (选中 instance 图层时)

属性面板顶部显示组件来源信息，可覆盖的属性标记 ◇：

- **组件名**: 显示 master 名称 + "进入 Master" 链接
- **Overrides**: 列出可覆盖属性（位置/尺寸/填充色/文字内容），修改后标记 ◇
- **Reset**: 单个属性可重置回 master 值
- 不可覆盖的属性（子树结构、Patch 连线）灰显
...
- 位置/尺寸合并为一个可折叠组，变换默认收起
- 点击分组标题 ▾ 箭头展开/收起
- 减少右面板滚动距离，曲线面板更容易到达

**自定义 ColorPicker**
- 替换原生 `<input type="color">`
- 色块点击弹出下拉面板：hex 输入框 + 30 色色板
- 暗色主题统一，支持直接输入 hex 值
...
底部面板，默认高度为视口的 1/3，可通过分割手柄拖拽调整（范围 120px ~ 60vh），高度自动持久化到 localStorage。
