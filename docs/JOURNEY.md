...
画像从能力图谱自动枚举 (含 component 分支):
∅ / {states} / {states,curves} / {states,patch} / {states,component} /
{states,curves,patch} / {states,component,curves} / {states,component,patch} / 全能力
...
3. 循环 {
     ...
     c. curl 执行这一步 → Read 截图查看结果
     d. 比对: 逐项检查截图是否符合预期 (见「截图深度检查」)
     e. 质疑: 有任何"不对"的感觉 → 停下来，追查原因
     f. 全部符合预期 → 回到 a
   }
...
> 这是旅程验证的核心能力。截图不是"看一眼确认没崩"，
> 而是逐区域、逐信号地对比实际画面与预期行为。
...
操作类型                  → 必须出现的视觉信号
──────────────────────────────────────────────────
编辑非默认状态的属性       → 橙色 "覆盖" badge + ◆ 菱形标记 + ↺ 重置按钮
编辑默认状态的属性         → 无 覆盖 badge、无 ◆ 菱形
切换状态标签              → 新状态标签高亮 (蓝紫底+下划线)
切换状态组 (group pill)   → 新 group pill 高亮 + 状态标签列表更新
创建组件                  → 画布新增一行 + 状态栏出现 group pill
Preview 自动循环          → 两个状态之间有可见的视觉变化
绘制图层                  → 画布上出现图层 + 图层面板新增条目
选中图层                  → 蓝紫色选中框 + 属性面板填充
```
...
```
异常指标:
- 属性面板在非默认状态下没有覆盖标记 → 🔴 写入路径可能错误
- 编辑属性后切回默认状态，值与编辑前不同 → 🔴 可能改了基础值
- Preview 循环两个状态无视觉差异 → 🔴 override 可能为空
- 图层面板数量与画布不一致 → 🔴 可能渲染丢失
- 属性面板值与画布视觉不匹配 → 🔴 渲染/数据不同步
```

**③ 逻辑是否自洽？(因果一致性)**
```
检查:
- 我刚修改了 Y=200，属性面板确实显示 Y=200 吗？
- 我在状态 2 修改了高度，切到状态 1 后高度回到原值了吗？
- 我创建了组件，状态栏是否从 1 行变成了 2 行？
- Preview 显示的状态名是否与我刚切换到的状态一致？
```

**④ 视觉是否一致？(UI 品质)**
```
每次看到属性面板/图层面板/工具栏时扫一眼:
- 同级区块的标题样式是否统一？(字号、透明度、间距、chevron)
- 输入框/按钮/色块的视觉权重是否一致？
- 间距节奏是否规律？(section 间距、row 间距、padding)
- 覆盖态指示器 (◆ 菱形、橙色边框、↺ 按钮) 是否全套出现/缺失？
- 有没有某个区域"看起来和旁边不太一样"？→ 停下来追查

典型案例 (已修复):
  - 属性面板中 "文本"/"外观" 用大号标题，
    "位置/尺寸"/"变换" 用 CollapsibleGroup 小号标题
    → 原因: .prop-label 定义在父组件 scoped CSS 中，
    无法穿透到子组件内部 → 标题实际无样式
```
...
```
发现异常 → 不要继续执行下一步操作
         → 用 evaluate 查询 Pinia store 数据确认
         → 对比 UI 显示与底层数据
         → 定位是渲染问题还是数据问题
         → 记录为摩擦点 (如确认是 bug 则标为 P0/P1)
```
...
```
❌ 错误做法 (实际发生的):
   "在状态 2 编辑了 Y=50、Height=400，截图显示属性面板的值确实变了" → 继续

✅ 正确做法 (应该做的):
   "在状态 2 编辑了 Y=50、Height=400"
   → 正信号检查: 属性面板应显示 ◆ 菱形标记 → 截图中有吗？没有！
   → 正信号检查: header 应显示 "覆盖" badge → 截图中有吗？没有！
   → 负信号检查: 切回默认状态看 Y/Height 是否恢复 → 没恢复！
   → 结论: 编辑走了 updateLayerProps 而非 setOverride，状态感知写入有 bug
```
...
```
检查项:
□ 覆盖验证: 非默认状态的属性修改生成了 override（用 evaluate 查 Pinia store）
□ 组件隔离: 组件 group 内的编辑不影响主画面数据
□ 导出完整: 导出文件 (.intera) 包含所有 group 的状态数据和 override
```
方法: 旅程中用 `evaluate` 检查 Pinia store 数据结构，不仅靠截图
摩擦类型: `data-integrity`
...
- ⛔ **交互修复必须做中间态验证** — 任何涉及拖拽/跟手/动画的修复，不能只验证编译通过和最终结果，**必须截图验证交互过程的中间帧** (如拖拽中途、动画进行时)。工具: journey-server 的 mousedown → mousemove → screenshot → mouseup 分步序列
