...
设计目标达成 → 体验审查 (见下方) → 归档旅程 (README.md + design.intera + 截图)
...
> 功能验证只回答"能不能用"，体验审查回答"好不好用"。两者都通过才能标 ✅。

**① 视觉一致性** — 截图逐项检查:
- 属性面板: 标签对齐、输入框样式统一、间距均匀
- Patch 编辑器: 节点大小一致、端口对齐、连线清晰
- 状态栏/工具栏: 激活态视觉反馈明确
- 字体层次、颜色对比度、间距一致性
...
**③ 交互品味** — 录制 1080p 视频，AI 先自审再发 kenefe:
- 操作是否符合直觉
- 反馈是否及时
- 整体是否像成熟产品
- 弹簧参数是否舒服

**铁律**: 体验问题和功能摩擦同等对待 — 记录 KNOWN-ISSUES.md，修复后重走验证。
**铁律**: 发视频前必须自己看一遍确认质量。
```
